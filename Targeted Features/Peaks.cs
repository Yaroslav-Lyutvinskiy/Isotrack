        ...
        public List<Peak> waveletPeakDetection(double minWidthTime, double maxWidthTime, double minIntensity, double baselineRatio)
        {
            ...
            while (true) {
                ...
                for (int j = 0; j < nEvents; j++) {
                    if (DataMax<Trace.Group.Points[j].Intensity){
                        DataMax = Trace.Group.Points[j].Intensity;
                    }
                }
                double[] ScaleMax = new double[nScales];
                for (int i = 0; i < nScales; i++) {
                    for (int j = 0; j < nEvents; j++) {
                        double Ratio=transform[i][j]/(Trace.Group.Points[j].Intensity/DataMax);
                        if (transform[i][j]>ScaleMax[i]){
                            ScaleMax[i] = transform[i][j];
                        }
                    }
                }
                ...
                //System.out.println("Transform max = " + maxt + " at scale = " + scales[bestScale] + ", event = " + bestEvent);
